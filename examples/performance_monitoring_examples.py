...
def example_6_bottleneck_identification():
    ...
    for metric in monitor.get_metrics():
        ...
        if percentage > 50:
            print(f"  ⚠️  {metric.operation} takes {percentage:.1f}% of total time - OPTIMIZE THIS!")
        elif percentage > 20:
            print(f"  ⚡ {metric.operation} takes {percentage:.1f}% of total time - consider optimizing")
        else:
            print(f"  ✓  {metric.operation} takes {percentage:.1f}% of total time - acceptable")
...
if __name__ == "__main__":
    ...
    print("="*80)
    print("\nKey Takeaways:")
    print("  1. Use PerformanceMonitor.track() for detailed operation-by-operation analysis")
