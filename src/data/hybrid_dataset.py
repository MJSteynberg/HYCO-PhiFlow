...
class HybridDataset(Dataset):
    ...
    def _convert_to_fields_with_start(
        self, data: Dict[str, Any], start_frame: int
    ) -> Tuple[Dict[str, Field], Dict[str, Field]]:
        """
        ...
        for name in self.field_names:
            ...
            initial_fields_dict[name] = converter.tensor_to_field(
                initial_tensors[name], initial_metadata[name], time_slice=0
            )
        ...
        for field_name in self.dynamic_fields:
            ...
            if isinstance(field_tensors_raw, torch.Tensor):
                field_tensors = (
                    field_tensors_raw.cuda()
                    if not field_tensors_raw.is_cuda
                    else field_tensors_raw
                )
            else:
    ...
    def __getitem__(
        ...
        Tuple[torch.Tensor, torch.Tensor], Tuple[Dict[str, Field], Dict[str, Field]]
    ]:
        """
        ...
        dynamic_data = torch.cat(
            dynamic_field_tensors, dim=1
        )  # [time, dynamic_channels, x, y]
