...
class MemoryMonitor:
    ...
    def print_memory_usage(prefix: str = "", device: int = 0):
        ...
        if torch.cuda.is_available():
            print(f"{prefix}CPU: {cpu_mem:.1f} MB | GPU Allocated: {gpu_mem:.1f} MB | GPU Reserved: {gpu_reserved:.1f} MB")
        else:
    ...
    def log_memory_usage(logger, prefix: str = "", device: int = 0):
        ...
        if torch.cuda.is_available():
            logger.info(f"{prefix}CPU: {cpu_mem:.1f} MB | GPU Allocated: {gpu_mem:.1f} MB | GPU Reserved: {gpu_reserved:.1f} MB")
        else:
...
class MemoryTracker:
    ...
    def print_summary(self):
        ...
        print("="*80)
        print(f"{'Label':<30} {'CPU (MB)':<12} {'GPU Alloc (MB)':<15} {'GPU Reserved (MB)':<15}")
        print("-"*80)
        ...
        for record in self.records:
            ...
            print(f"{label:<30} {cpu:>10.1f}   {gpu_alloc:>13.1f}   {gpu_reserved:>17.1f}")
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs) -> Any:
            ...
            if verbose:
                cpu_delta = end_cpu - start_cpu
                gpu_delta = end_gpu - start_gpu
                print(f"[Memory] {func_label}: CPU {cpu_delta:+.1f} MB, GPU {gpu_delta:+.1f} MB")
                if torch.cuda.is_available():
                    peak_gpu = torch.cuda.max_memory_allocated(device) / 1024 / 1024
                    print(f"         Peak GPU: {peak_gpu:.1f} MB")
...
class EpochMemoryMonitor:
    ...
    def on_batch_end(self, batch_idx: int, loss: Optional[float] = None):
        ...
        print(f"  Batch {batch_idx}: GPU {gpu_mem:.1f} MB, CPU {cpu_mem:.1f} MB{loss_str}")
    ...
    def on_epoch_end(self):
        ...
        if torch.cuda.is_available():
            ...
            print(f"  Epoch end: GPU {current_gpu:.1f} MB allocated, "
...
class BatchMemoryProfiler:
    ...
    def __init__(self, enabled: bool = True, device: int = 0):
        ...
        self._last_gpu = 0.0 if not torch.cuda.is_available() else MemoryMonitor.get_gpu_memory_mb(device)
...
class PerformanceMetrics:
    ...
    def __str__(self):
        return (f"{self.operation}: {self.duration_seconds:.3f}s, "
...
class PerformanceMonitor:
    ...
    def print_summary(self, show_aggregate: bool = True):
        ...
        print("="*90)
        print(f"{'Operation':<30} {'Time (s)':<12} {'CPU (MB)':<15} {'GPU (MB)':<15} {'GPU Peak':<12}")
        print("-"*90)
        
        for m in self.metrics:
            cpu_str = f"{m.cpu_delta_mb:+.1f}".rjust(7)
            gpu_str = f"{m.gpu_delta_mb:+.1f}".rjust(7)
            print(f"{m.operation:<30} {m.duration_seconds:>10.3f}  {cpu_str:>13}  {gpu_str:>13}  {m.gpu_peak_mb:>10.1f}")
        ...
        if show_aggregate and self.aggregate_metrics:
            ...
            print("="*90)
            print(f"{'Operation':<30} {'Count':<8} {'Total (s)':<12} {'Mean (s)':<12} {'Min (s)':<12} {'Max (s)':<12}")
            print("-"*90)
            ...
            for op_name, durations in self.aggregate_metrics.items():
                ...
                print(f"{op_name:<30} {count:<8} {total:<12.3f} {mean:<12.3f} {min_dur:<12.3f} {max_dur:<12.3f}")
...
def track_performance(operation: str, print_result: bool = True, device: int = 0):
    ...
    try:
        ...
        if print_result:
            print(f"[Performance] {operation}: {duration:.3f}s, CPU {cpu_delta:+.1f}MB, GPU {gpu_delta:+.1f}MB")
            if torch.cuda.is_available():
...
def monitor_performance(operation: Optional[str] = None, device: int = 0):
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs) -> Any:
            ...
            print(f"[Performance] {op_name}: {duration:.3f}s, CPU {cpu_delta:+.1f}MB, GPU {gpu_delta:+.1f}MB")
            if torch.cuda.is_available():
                gpu_peak = torch.cuda.max_memory_allocated(device) / 1024 / 1024
                print(f"              GPU peak: {gpu_peak:.1f}MB")
...
class EpochPerformanceMonitor:
    ...
    def on_batch_end(self, batch_idx: int, loss: Optional[float] = None):
        ...
        if batch_idx < self.verbose_batches:
            ...
            loss_str = f", loss={loss:.6f}" if loss is not None else ""
            print(f"  Batch {batch_idx}: {batch_time:.3f}s, GPU {gpu_mem:.1f}MB, CPU {cpu_mem:.1f}MB{loss_str}")
    ...
    def on_epoch_end(self):
        ...
        if torch.cuda.is_available():
            ...
            print(f"  Epoch end: {epoch_time:.3f}s total, GPU {current_gpu:.1f}MB allocated, "
                  f"{reserved_gpu:.1f}MB reserved, peak {peak_gpu:.1f}MB")
        else:
        ...
        if self._batch_times:
            ...
            print(f"  Batch timing: avg={avg_batch:.3f}s, min={min_batch:.3f}s, max={max_batch:.3f}s")
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'avg_epoch_time': sum(self._epoch_times) / len(self._epoch_times) if self._epoch_times else 0.0,
