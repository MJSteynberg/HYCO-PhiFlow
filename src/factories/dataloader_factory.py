...
class DataLoaderFactory:
    ...
    def create(
        ...
        sim_indices = sim_indices if sim_indices is not None else cfg.get_train_sim_indices()
        batch_size = batch_size if batch_size is not None else cfg.get_batch_size()
        use_sliding_window = use_sliding_window if use_sliding_window is not None else cfg.should_use_sliding_window()
        enable_augmentation = enable_augmentation if enable_augmentation is not None else cfg.is_augmentation_enabled()
        ...
        if mode == 'tensor':
            ...
            raise ValueError(
                f"Unknown mode: {mode}. Must be 'tensor' or 'field'."
            )
