...
class TestSmokeModel:
    ...
    def test_initialization_with_random_inflow(self, basic_config):
        ...
        config["pde_params"].pop(
            "inflow_center"
        )  # Remove inflow_center to trigger random generation
    ...
    def test_different_domains(self, basic_config):
        ...
        for domain in domains:
            ...
            config["pde_params"]["inflow_center"] = (
                domain["size_x"] / 2,
                domain["size_y"] / 4,
            )
            model = SmokeModel(config)
    ...
    def test_numerical_stability(self, model):
        ...
        for _ in range(50):
            ...
            assert not math.any(
                math.is_nan(velocity_values)
            ).all, "NaN detected in velocity"
            assert not math.any(
                math.is_inf(velocity_values)
            ).all, "Inf detected in velocity"
            assert not math.any(
                math.is_nan(density_values)
            ).all, "NaN detected in density"
            assert not math.any(
                math.is_inf(density_values)
            ).all, "Inf detected in density"
    ...
    def test_inflow_persistence(self, model):
        ...
        assert math.all(
            math.close(
                initial_inflow, final_inflow, rel_tolerance=1e-6, abs_tolerance=1e-6
            )
        )
