...
class TestFieldTensorConverterInitialization:
    ...
    def test_init_multiple_fields(self, scalar_metadata, vector_metadata):
        ...
        converter = FieldTensorConverter(
            {"density": scalar_metadata, "velocity": vector_metadata}
        )
...
class TestScalarFieldConversion:
    ...
    def test_scalar_field_to_tensor_no_batch(
        self, simple_domain, simple_resolution, scalar_metadata
    ):
    ...
    def test_scalar_field_to_tensor_with_batch(
        self, simple_domain, simple_resolution, scalar_metadata
    ):
    ...
    def test_roundtrip_scalar_field(
        self, simple_domain, simple_resolution, scalar_metadata
    ):
        ...
        reconstructed_native = math.reshaped_native(
            reconstructed_field.values, ["x", "y"]
        )

        assert torch.allclose(original_native, reconstructed_native, atol=1e-5)


class TestVectorFieldConversion:
    """Test conversion of vector fields."""

    def test_vector_field_to_tensor_no_batch(
        self, simple_domain, simple_resolution, vector_metadata
    ):
    ...
    def test_vector_field_to_tensor_with_batch(
        self, simple_domain, simple_resolution, vector_metadata
    ):
        ...
        field = CenteredGrid(
            math.tensor(
                vector_data,
                batch_dim("batch") & simple_resolution & channel(vector="x,y"),
            ),
            extrapolation.PERIODIC,
            bounds=simple_domain,
        )
    ...
    def test_roundtrip_vector_field(
        self, simple_domain, simple_resolution, vector_metadata
    ):
        ...
        original_native = math.reshaped_native(
            original_field.values, ["x", "y", "vector"]
        )
        reconstructed_native = math.reshaped_native(
            reconstructed_field.values, ["x", "y", "vector"]
        )
...
class TestMultiFieldConversion:
    ...
    def test_multiple_fields_to_tensor(
        self, simple_domain, simple_resolution, scalar_metadata, vector_metadata
    ):
        ...
        velocity_field = CenteredGrid(
            math.tensor(
                torch.ones(32, 32, 2) * 2, simple_resolution & channel(vector="x,y")
            ),
            extrapolation.PERIODIC,
            bounds=simple_domain,
        )
    ...
    def test_tensor_to_multiple_fields(self, scalar_metadata, vector_metadata):
        ...
        velocity_values = math.reshaped_native(
            fields["velocity"].values, ["x", "y", "vector"]
        )
        assert torch.all(velocity_values[..., 0] == 2.0)  # x component
        assert torch.all(velocity_values[..., 1] == 3.0)  # y component

    def test_roundtrip_multiple_fields(
        self, simple_domain, simple_resolution, scalar_metadata, vector_metadata
    ):
        ...
        density_orig = math.reshaped_native(
            original_fields["density"].values, ["x", "y"]
        )
        density_recon = math.reshaped_native(
            reconstructed_fields["density"].values, ["x", "y"]
        )

        velocity_orig = math.reshaped_native(
            original_fields["velocity"].values, ["x", "y", "vector"]
        )
        velocity_recon = math.reshaped_native(
            reconstructed_fields["velocity"].values, ["x", "y", "vector"]
        )
...
class TestBatchedConversion:
    ...
    def test_batched_multiple_fields(
        self, simple_domain, simple_resolution, scalar_metadata, vector_metadata
    ):
    ...
    def test_batched_roundtrip(
        ...
        density_orig = math.reshaped_native(
            original_fields["density"].values, ["batch", "x", "y"]
        )
        density_recon = math.reshaped_native(
            reconstructed_fields["density"].values, ["time", "x", "y"]
        )  # batch becomes 'time'

        velocity_orig = math.reshaped_native(
            original_fields["velocity"].values, ["batch", "x", "y", "vector"]
        )
        velocity_recon = math.reshaped_native(
            reconstructed_fields["velocity"].values, ["time", "x", "y", "vector"]
        )
...
class TestValidation:
    ...
    def test_validate_fields_correct(
        self, simple_domain, simple_resolution, scalar_metadata, vector_metadata
    ):
        ...
        fields = {
            ...
            "velocity": CenteredGrid(
                math.tensor(
                    torch.ones(32, 32, 2), simple_resolution & channel(vector="x,y")
                ),
                extrapolation.PERIODIC,
                bounds=simple_domain,
            ),
    ...
    def test_validate_fields_wrong_names(
        self, simple_domain, simple_resolution, scalar_metadata
    ):
