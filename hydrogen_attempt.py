...
def euler_step(v, r_ng, r_h2, dt=DT):
    ...
    # --- 6. Stabilization (Artificial Viscosity) ---
    # Compressible Euler is unstable (shocks). We add slight diffusion.
    # v_new = diffuse.explicit(v_new, DAMPING, dt)
    # Inside euler_step function, add this line near the end:
    # We add a small noise tensor (0.1% of max speed) to the velocity
    noise = CenteredGrid(Noise(v_new.values.shape, scale=10, smoothness = 1), x=RES[0], y=RES[1], bounds=ARENA_SIZE)
    v_final = field.where(inflow_mask, target_velocity + noise * 0.1 * target_velocity, v_new)
...
print("🚀 Running Compressible Euler Simulation...")
